...
[저번 포스팅](https://seongjaemoon.github.io/2017/01/27/database-oracle5/)을 통해 오라클의 간단한 쿼리문에 대해서 알아보았다. 오라클은 RDMBS 중에서도 진짜 잘 만들어진 DBMS로서 다양한 기능을 제공하고 있다. 검색 쿼리 최적화라던지 이런 저런 기능들은 너무 종류가 많아서 다 정리하기 벅차므로 API 문서 등을 열심히 찾아보는 걸로 하고, Oracle 11g와 OJDBC를 통해 JAVA 환경에서 실제 데이터베이스를 활용하는 방법에 대해서 정리하고자 한다.  
...
또한, SQL 쿼리문을 테스트 하고 실제 오라클 데이터베이스에 튜플을 확인하기 위해 SQL developer라는 프로그램 사용한다.
...
![Docker 실행](/uploads/docker.png)
Docker를 다운로드 받고 실행을 하면, 상태바에 고래 모양을 띈 귀여운 친구가 나타난 것을 확인할 수 있다. 고래를 클릭하면 Docker is running 이라는 창이 나오면 준비가 완료된다.
...
![SQL developer SELECT 쿼리 테스트](/uploads/sqldev2.png)
