...
#### 위성 데이터
우선, 위성 데이터를 제대로 수신 받기 위해선 각각 최소 4개 이상의 공통 위성에서 측정된 값을 동시에 저장하여 각 측점 간 벡터기선을 계산해주어야 한다. 그렇기 때문에 최대한 많은 위성의 신호를 받을 수 있는 시간대와 위치를 선점하여 수신을 실시해야 한다. 현재, 많은 위성이 일정한 간격으로 지구전체를 돌고 있으므로 지구상 어느 곳 에서든지 항상 4개 이상의 GPS 신호를 잡을 수 있다 :)
