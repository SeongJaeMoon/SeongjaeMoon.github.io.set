...
[저번 포스팅에](https://seongjaemoon.github.io/2018/03/06/algorithmClock/)에 이어서 알고리즘 문제 해결 책의 새로운 챕터를 맞이하여, 문제를 풀기 전에 정리되어 있는 분할 정복의 도입 부분과 예제 풀이 부분 등에 대해 정리해보도록 하자.
...
$={n \over 2} * {n \over 2} + (1 + 2 + 3 + ... + {n\over 2})$

$={n \over 2} * {n \over 2} + fastSum({n \over 2})$
...
$fastSum(n) = 2 * fastSum({n \over 2}) + {n^2 \over 4}$ (n이 짝수일 때)
...
### 행렬의 빠른 제곱.
n x n 크기의 행렬 A가 주어질 때, A의 거듭제곱(pow)$A^m$은 A를 연속해서 m번 곱한 것이다. 이것을 계산하는 알고리즘 자체는 어려울 것이 없지만, m이 매우 클 때 A를 구하는 것은 꽤나 시간이 오래 걸리는 작업이 된다. 행렬의 곱셈에는 일반적으로 O($n^2$)의 시간이 들기 때문에 곧이곧대로 m - 1번의 곱셈을 통해 $A^m$을 구하려면 모두 O($n^2 * m$)의 연산이 필요하다. 절레절레... 하지만 분할 정복을 이용하는 눈 깜짝할 새에 연산이 가능하다.

- $A^m = A^{n \over 2} * A^{n \over 2}$
...
$a \cdot b = (a_1 x 10^{128} + a_0) x (b_1 \cdot 10^{128} + b_0)$
...
다음 알고리즘 포스팅부턴 본격적으로 불한 정복 알고리즘에 대해 알아보는 걸로~
